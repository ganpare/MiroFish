    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            <template v-if="graphData">
              ...
              <span class="stat-item">{{ graphData.edge_count || graphData.edges?.length || 0 }} リレーション</span>
              <span class="stat-divider">|</span>
            </template>
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">グラフデータを読み込んでいます...</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-hint">データはまもなく表示されます...</p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          ...
          <span class="header-title">構築プロセス</span>
        </div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  ドキュメントをアップロード後、LLMがドキュメント内容を分析し、世論シミュレーションに適したオントロジー構造（エンティティタイプ + リレーションタイプ）を自動生成します
                </div>
              </div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                ...
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    +{{ projectData.ontology.relation_types.length - 5 }} 個の追加の関係...
                  </div>
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">インターフェース説明</div>
                <div class="detail-content">
                  生成されたオントロジーに基づき、ドキュメントを分割後にZep APIを呼び出して知識グラフを構築し、エンティティと関係を抽出します
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">次のステップへの準備完了</div>
              </div>
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'アップロード待ちのファイルがありません。ホームページに戻ってから再度お試しください'
    loading.value = false
    return
  }
  ...
  try {
    ...
    ontologyProgress.value = { message: 'ファイルをアップロードし、ドキュメントを分析しています...' }
    ...
    if (response.success) {
      ...
      error.value = response.error || 'オントロジーの生成に失敗しました'
    }
  } catch (err) {
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = 'グラフ構築タスクが開始されました...'
      ...
      error.value = response.error || 'グラフ構築の開始に失敗しました'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'グラフ構築の開始に失敗しました: ' + (err.message || '不明なエラー')
    buildProgress.value = null
  }
}
