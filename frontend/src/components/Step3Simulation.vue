    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? '生成中...' : '結果レポートを生成' }} 
    ...
    <div class="main-content-area" ref="scrollContainer">
      ...
      <div class="timeline-header" v-if="allActions.length > 0">
        <div class="timeline-stats">
          <span class="total-count">総イベント数: <span class="mono">{{ allActions.length }}</span></span>
              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'REPOST'">
                  <div class="repost-info">
                    ...
                    <span class="repost-label">@{{ action.action_args?.original_author_name || 'User' }} の投稿を再共有</span>
                  </div>
                ...
                <template v-if="action.action_type === 'LIKE_POST'">
                  <div class="like-info">
                    ...
                    <span class="like-label">@{{ action.action_args?.post_author_name || 'User' }} の投稿にいいね</span>
                  </div>
                ...
                <template v-if="action.action_type === 'CREATE_COMMENT'">
                  ...
                  <div v-if="action.action_args?.post_id" class="comment-context">
                    ...
                    <span>投稿 #{{ action.action_args.post_id }} に返信</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'SEARCH_POSTS'">
                  <div class="search-info">
                    ...
                    <span class="search-label">検索クエリ:</span>
                ...
                <template v-if="action.action_type === 'FOLLOW'">
                  <div class="follow-info">
                    ...
                    <span class="follow-label">@{{ action.action_args?.target_user || action.action_args?.user_id || 'User' }} をフォロー</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'UPVOTE_POST' || action.action_type === 'DOWNVOTE_POST'">
                  <div class="vote-info">
                    ...
                    <span class="vote-label">{{ action.action_type === 'UPVOTE_POST' ? '高評価' : '低評価' }}した投稿</span>
                  </div>
                ...
                <template v-if="action.action_type === 'DO_NOTHING'">
                  <div class="idle-info">
                    ...
                    <span class="idle-label">アクションなし</span>
                  </div>
                </template>
...
const doStartSimulation = async () => {
  ...
  startError.value = null
  addLog('デュアルプラットフォームの並行シミュレーションを起動しています...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✗ 起動に失敗しました: ${res.error || '不明なエラー'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ 起動で例外が発生しました: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`停止に失敗しました: ${res.error || '不明なエラー'}`)
    }
  } catch (err) {
    addLog(`停止で例外が発生しました: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ 全てのプラットフォームでシミュレーションが終了したことを検知しました')
        }
    ...
    console.warn('実行ステータスの取得に失敗しました:', err)
  }
}
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    console.warn('詳細ステータスの取得に失敗しました:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('レポート生成リクエストを送信しました、少々お待ちください...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('レポートの生成を開始しています...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ レポート生成タスクを開始しました: ${reportId}`)
      
      // 跳转到报告页面
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ レポート生成の開始に失敗しました: ${res.error || '不明なエラー'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ レポート生成の開始で例外が発生しました: ${err.message}`)
    isGeneratingReport.value = false
  }
}
