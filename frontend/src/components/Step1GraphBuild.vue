    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            生成されたオントロジーに基づき、ドキュメントを自動でチャンク分割してからZepを呼び出し、ナレッジグラフを構築します。エンティティと関係を抽出し、時系列メモリとコミュニティサマリーを形成します
          </p>
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 2, 'completed': currentPhase >= 2 }">
        ...
        <div class="card-content">
          ...
          <p class="description">グラフ構築が完了しました。次のステップに進んでシミュレーション環境を構築してください</p>
          <button 
            class="action-btn" 
...
const handleEnterEnvSetup = async () => {
  ...
  try {
    ...
    if (res.success && res.data?.simulation_id) {
      ...
      console.error('シミュレーションの作成に失敗しました:', res.error)
      alert('シミュレーションの作成に失敗しました: ' + (res.error || '不明なエラー'))
    }
  } catch (err) {
    console.error('シミュレーションの作成で例外が発生しました:', err)
    alert('シミュレーションの作成で例外が発生しました: ' + err.message)
  } finally {
