...
if os.path.exists(_env_file):
    ...
    print(f"環境設定を読み込みました: {_env_file}")
else:
    # 尝试加载 backend/.env
    _backend_env = os.path.join(_backend_dir, '.env')
    if os.path.exists(_backend_env):
        load_dotenv(_backend_env)
        print(f"環境設定を読み込みました: {_backend_env}")
...
try:
    ...
    print("先に `pip install oasis-ai camel-ai` を実行してください")
    sys.exit(1)
            ...
            try:
                ...
                for interview in twitter_interviews:
                    ...
                    try:
                        ...
                        print(f"  Warning: could not access Twitter agent {agent_id}: {e}")
            ...
            try:
                ...
                for interview in reddit_interviews:
                    ...
                    try:
                        ...
                        print(f"  Warning: could not access Reddit agent {agent_id}: {e}")
...
def fetch_new_actions_from_db(
    ...
    try:
        ...
        print(f"データベースからアクションを読み込めませんでした: {e}")
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("OPENAI_API_KEY is missing. Set LLM_API_KEY in the project root .env file.")
...
async def run_twitter_simulation(
    ...
    if not os.path.exists(profile_path):
        log_info(f"エラー: Profile ファイルが見つかりません: {profile_path}")
        return result
    ...
    if initial_posts:
        ...
        if initial_actions:
            ...
            log_info(f"初期投稿を {len(initial_actions)} 件投入しました")
    ...
    if max_rounds is not None and max_rounds > 0:
        ...
        if total_rounds < original_rounds:
            log_info(f"ラウンド数を制限しました: {original_rounds} -> {total_rounds} (max_rounds={max_rounds})")
    ...
    for round_num in range(total_rounds):
        ...
        if _shutdown_event and _shutdown_event.is_set():
            if main_logger:
                main_logger.info(f"終了シグナルを受信したため、第 {round_num + 1} ラウンドで停止します")
            break
    ...
    log_info(f"シミュレーションを完了しました。所要時間: {elapsed:.1f} 秒, 総アクション数: {total_actions}")
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum rounds to execute (optional)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close environments immediately after the run instead of waiting for interview commands'
    )
    ...
    log_manager.info("=" * 60)
    log_manager.info("OASIS 並列シミュレーション")
    log_manager.info(f"設定ファイル: {args.config}")
    log_manager.info(f"シミュレーションID: {config.get('simulation_id', 'unknown')}")
    log_manager.info(f"待機コマンドモード: {'有効' if wait_for_commands else '無効'}")
    log_manager.info("=" * 60)
    ...
    log_manager.info("シミュレーション設定:")
    log_manager.info(f"  - 総シミュレーション時間: {total_hours} 時間")
    log_manager.info(f"  - 1 ラウンドあたり: {minutes_per_round} 分")
    log_manager.info(f"  - 設定上の総ラウンド数: {config_total_rounds}")
    if args.max_rounds:
        log_manager.info(f"  - 最大ラウンド制限: {args.max_rounds}")
        if args.max_rounds < config_total_rounds:
            log_manager.info(f"  - 実行ラウンド数: {args.max_rounds} (短縮済み)")
    log_manager.info(f"  - Agent 数: {len(config.get('agent_configs', []))}")
    
    log_manager.info("ログ出力:")
    log_manager.info("  - メインログ: simulation.log")
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"シミュレーションが完了しました。総所要時間: {total_elapsed:.1f} 秒")
    
    # 是否进入等待命令模式
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("コマンド待機モードに入ります。環境は起動したままです")
        log_manager.info("利用可能コマンド: interview, batch_interview, close_env")
        log_manager.info("=" * 60)
    ...
    if twitter_result and twitter_result.env:
        ...
        log_manager.info("[Twitter] 環境を終了しました")
    
    if reddit_result and reddit_result.env:
        await reddit_result.env.close()
        log_manager.info("[Reddit] 環境を終了しました")
    
    log_manager.info("=" * 60)
    log_manager.info("すべて完了しました")
    log_manager.info("ログファイル:")
