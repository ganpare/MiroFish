...
def canonicalize_entity_type(raw_type: Optional[str]) -> str:
    ...
    person_tokens = {
        "person", "people", "individual", "actor", "leader", "celebrity", "expert",
        "scholar", "journalist", "student", "citizen", "witness", "victim",
        "perpetrator", "influencer", "opinionleader", "kols", "kol",
    }
    org_tokens = {
        "organization", "org", "company", "enterprise", "brand", "agency", "department",
        "government", "regulator", "university", "school", "institute", "ngo",
        "union", "association", "foundation", "media", "newspaper", "tv", "platform",
        "committee", "community", "account",
    }
    product_tokens = {
        "product", "app", "application", "service", "tool", "model", "software",
        "system", "api", "framework", "device", "game",
    }
    location_tokens = {
        "location", "place", "city", "country", "province", "region", "state",
        "county", "district", "area",
    }

    tokens = {part for part in re.split(r"[^a-z0-9]+", tl) if part}
