...
def _get_default_agentizable_types(project) -> Optional[List[str]]:
    """Read default agentizable types from the project's ontology."""
    ontology = getattr(project, "ontology", None) or {}
    agentizable_types = ontology.get("agentizable_types")
    if not isinstance(agentizable_types, list):
        return None
    normalized = [item for item in agentizable_types if isinstance(item, str) and item.strip()]
...
def prepare_simulation():
    ...
    try:
        ...
        entity_types_list = data.get('entity_types') or _get_default_agentizable_types(project)
